...
export default function ReportsPage({ variant = "full", embedded = false }) {
  ...
  const [monthlyBudgetSectionOpen, setMonthlyBudgetSectionOpen] = useState(false);
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              {monthlyBudgetSectionOpen ? (
                ...
                <p className="text-xs text-th-muted mt-0.5">
                  Cap, alerts, and category lines for this month — press Show to edit.
                </p>
              )}
            </div>
            <div className="flex flex-wrap items-center gap-2 shrink-0">
              <button
                type="button"
                onClick={() => setMonthlyBudgetSectionOpen((v) => !v)}
                className="text-th-muted text-xs hover:text-th-tertiary px-1 py-0.5 rounded"
                aria-expanded={monthlyBudgetSectionOpen}
              >
                {monthlyBudgetSectionOpen ? "Hide" : "Show"}
              </button>
              <button
                type="button"
