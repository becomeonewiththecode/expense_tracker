...
On the **Expenses** page, you can **add an expense manually** (same fields as on **Import**) when the list is empty, or expand **Add expense manually** when you already have rows. The table lists saved expenses (newest first). In **modification mode**, row **Edit** lets you change **transaction date**, amount, category, frequency, institution, **State**, and note.

**Renewal reminders:** For recurring expenses (**Weekly**, **Monthly**, **Bi-monthly**, **Yearly**), the app estimates the next renewal from **frequency** and **transaction date** (day and month of year come from that date). While you are signed in, an **Upcoming renewals** panel may appear at the top of the main content on **Import**, **Expenses**, **Reports**, and on **Profile** (open **Profile** from the avatar **account menu**), when the next renewal is **25–40**, **15–24**, or **0–14** whole calendar days away (roughly a month, a couple of weeks, or the final two weeks—the wording may say “about 30 days,” “about 15 days,” or an exact count such as “in 12 days”). Reminders are **grouped by financial institution** (Bank, VISA, and so on): each group has its own table (expense, transaction date, amount, **State**, renewal date), a **Subtotal** for that institution (**Active** rows only), and a **Total (all institutions)** line at the bottom (also **Active** only—the same per-line amounts as on your expense list, excluding **Cancel** from the sums). Lines marked **Cancel** in **State** are shown with a **green** background so you can spot subscriptions you have cancelled while still seeing the computed renewal date. After a renewal date has passed, a line is hidden for a short period until the next occurrence is nearer (the earliest &quot;about a month out&quot; window does not apply for about two weeks after that date, so the list does not immediately show the following cycle). **One-time** rows are ignored. Use **Dismiss** to hide a line for this browser session, or **Dismiss all** to hide every visible reminder. If you dismiss **all** rows but qualifying renewals still exist, a **number** appears on the profile avatar in the header; open the **account menu** (avatar) and choose **Upcoming renewals** to show the full panel again and clear those dismissals for this session. **Sign out** is in the same menu. The count stays visible as you navigate between pages until you restore the panel or **sign out**. Reminders are **in-app only**—not email or push notifications.
...
The header shows **Import**, **Expenses**, and **Reports** as the main navigation; there is no separate **Profile** tab. Signed-in users open **Profile** from the **account menu** (click the avatar in the header) to update **email**, **password**, and **profile picture**. **Recovery code** (under **Password recovery**): generate a code once, store it safely offline, and use it on **`/recover`** if you forget your password. The **full code is shown only at the moment you create or replace it**; afterward, Profile shows a **masked placeholder** so you can see that a code is on file without seeing the secret. Replacing or removing the code invalidates the previous one.

**Backup and restore:** Download a **JSON** file of all expenses (`expense-tracker-backup` format). Each expense object includes **`spent_at`**, **`frequency`**, **`state`** (`active` or `cancel`), category, institution, amount, description, and denormalized **`payment_day`** / **`payment_month`** (derived from **`spent_at`** on save and restore). Older backup files without **`state`** still restore successfully (**Active** is assumed). **Restore** accepts that same file: **Append** adds imported rows to existing data; **Replace** deletes all current expenses first, then imports the file (use with care). Each restore is limited to **25,000** expense rows and a **15 MB** request body. Backups may include your account **email** in metadata—store files securely. If **Download backup** or **Restore** reports **Invalid token**, try **Continue session** if a prompt appears; otherwise **sign out** and **sign in** again (or the server’s signing secret may have changed).
