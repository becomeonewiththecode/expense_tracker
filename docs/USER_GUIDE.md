...
**Sign out** clears your session in the browser (you will need to sign in again to use **Import** and **Lists**). If your **JSON Web Token** has expired but is still within the server’s refresh window, you may be offered **Continue session** instead of only seeing errors—see [Session and security](#session-and-security).
...
- **When you already have expenses:** the **Import** page shows **Import from statement** and optional **Add expense manually** (expand the section). Open **Lists** in the header, then **Expenses** (or use the button on **Import**) to view, edit, and delete saved rows on a separate page. After your **first** saved expense (from the onboarding form) or after you **commit** an import that adds rows, you are taken to **Expenses** automatically.
