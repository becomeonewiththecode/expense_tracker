...
4. Click **Add categorized rows to expenses**. Committed lines are saved with **State** **Active**; change **State** later under **Your expenses** if a row is cancelled. **Discard import** deletes the staged batch without saving expenses.
...
The **Reports** page shows **charts** of spending for different periods. The same **Upcoming renewals** panel (when applicable) appears above the report content as on other tabs.
