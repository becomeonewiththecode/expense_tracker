...
| **Renewals page** | Client route **`/renewals`** (`RenewalsPage.jsx`). Lists **`GET /api/expenses?category=renewal`**. Same table patterns as **Expenses** (sort, modification mode, projection, delete). |
...
| Shared table + renewal columns | `client/src/components/ExpenseTable.jsx` |
