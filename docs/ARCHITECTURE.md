...
- **React Router version 6** — Public routes include `/login`, `/register`, **`/recover`** (password reset with a saved recovery code; no email is sent), and **`/oauth/callback`** (return URL after single sign-on). The `/` route is handled by an auth-aware shell: signed-out visitors see **`LandingPage`**; signed-in users enter the authenticated **`Layout`** shell. For signed-out users, non-public app routes (for example `/reports` or `/budget`) redirect to `/login`. Authenticated users use routes such as `/`, `/expenses`, `/expenses/list`, **`/income`**, **`/savings`**, **`/renewals`**, **`/prescriptions`**, **`/payment-plans`**, **`/budget`**, `/reports`, and **`/profile`**. The **`Layout`** header lists **Import** and the list routes: below the **`lg`** breakpoint they sit under a **Lists** dropdown; at **`lg`** and up they appear as **NavLink**s (**Income**, **Savings**, **Expenses**, **Renewals**, **Prescriptions**, **Payment Plan**, **Budget** in that order). There is no top-level **Reports** link; **`ReportsPage`** is reached via **`/reports`** or embedded inside **`BudgetHubPage`**. **Profile**, **Upcoming expenses** (when applicable), and **Sign out** are in the **avatar account menu** (not top-level tabs); **theme** is set under **Profile** → **Appearance**, not in the menu. **Post-login navigation:** the app calls `GET /api/expenses?limit=1`. If at least one expense exists, the user is sent to **`/expenses/list`**; otherwise to **`/expenses`**. The same rule applies for the index route `/`, successful login or registration, successful OAuth, and visits to `/login` while already signed in.  
