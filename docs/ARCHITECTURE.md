...
| Category (**including `renewal`** and **`payment_plan`**), **`renewal_kind`**, institution, frequency, **state** enums; **`spent_at`** → **`payment_day`** / **`payment_month`** | `server/src/expenseEnums.js` |
| Payment plan enums and validation | `server/src/paymentPlanEnums.js` |
