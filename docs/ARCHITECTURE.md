...
- **`PaymentPlansPage`** at **`/payment-plans`** — CRUD for **`payment_plans`** (**`GET`/`POST`/`PATCH`/`DELETE /api/payment-plans`**). Tracks category, payment schedule, priority, status, account type, payment method, institution, tag, frequency, amount, and notes. If account type is **Credit Card**, institution options are **VISA**, **American Express**, and **Mastercard**. Header includes **Search notes** (filters by notes) and a brief **update flash** after successful save/add updates. Expense rows with category **`payment_plan`** are synchronized to this table through **`source_expense_id`**.
