...
**`user_id`** references **`users`**. **`name`**, **`amount`**, **`renewal_period`** (allow-list: **`one_month`** through **`eleven_months`**, **`one_year`** through **`five_years`**), **`next_renewal_date`**, **`vendor`**, **`notes`**, **`category`**, **`state`** (**`active`** / **`cancel`**), **`created_at`**. Not part of expense **backup** JSON unless extended later. See [**PRESCRIPTIONS.md**](./PRESCRIPTIONS.md).
