...
- **`ExpensesPage`** — Titled **Import** in navigation; onboarding when there are no saved expenses; later, import plus optional collapsible manual entry; links to the list page. **`YourExpensesPage`** at `/expenses/list` — Table of expenses with modification mode (transaction date, amount, category, frequency, institution, **state**, note); manual add (same fields as Import) when empty or under **Add expense manually**; empty state also links to **Import**. Statement import staging lives on **Import**, then commit.  
- **`ReportsPage`** — Tabbed report types, fetches report endpoints, shows monthly summary list.  
- **`Layout`** — Primary nav (**Import**, **Your expenses**, **Reports**); the avatar **account menu** includes **Profile**, optional **Upcoming renewals** (when reminders are fully dismissed but still qualify), and **Sign out**; renders **`RenewalReminders`** above the outlet on every shell route (fetches **`GET /api/expenses?limit=500`**, filters recurring rows by **`renewalReminderTier`**, shows **one table per financial institution** with **Subtotal** rows and a **Total (all institutions)** band (**Active** amounts only; **Cancel** excluded), per-row and **Dismiss all** with keys in **`sessionStorage`**). When every visible row is dismissed but eligible renewals remain, **`RenewalReminders`** reports a **count** and expand callback to **`Layout`**, which shows a **number** on the avatar and an **Upcoming renewals** item in that menu; choosing it clears dismiss state and restores the panel.  
...
| Renewal date math, grouped reminder UI, avatar badge + account menu when fully dismissed | `client/src/renewalSchedule.js`, `client/src/components/RenewalReminders.jsx`; mounted from **`Layout.jsx`** on all authenticated shell routes |
