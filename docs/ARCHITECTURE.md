...
- **Cross-Origin Resource Sharing** uses an explicit allowlist from **`CLIENT_ORIGIN`** (comma-separated origins allowed). If **`CLIENT_ORIGIN`** is unset, development defaults to **`http://localhost:5173`** and **`http://127.0.0.1:5173`**; production with an unset variable yields an **empty** allowlist so credentialed browser calls from unknown origins are denied until you configure **`CLIENT_ORIGIN`**. **`credentials: true`** remains enabled for cookie-ready clients; the SPA uses **`Authorization`** headers from **`localStorage`**. **Per-IP rate limits** apply to **`POST /auth/register`**, **`POST /auth/login`**, **`POST /auth/oauth/login-code`**, and **`POST /api/admin/auth/login`** (see **`server/src/rateLimit.js`**).
