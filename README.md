...
**Documentation index:** [User guide](./docs/USER_GUIDE.md) (accounts, import, shell nav: **Lists** ▾ on small viewports or inline **Expenses** / **Renewals** / **Prescriptions** / **Payment Plan** / **Reports** links from **`lg`** up, **Expenses** list excluding renewal-category rows, **recovery codes**, **backup/restore** with **`account`** labeling and optional **`recoveryCode`** in JSON, **expired-session refresh**, **production Compose** / **`ensure-env` JWT bootstrap**, expense **State** Active/Paused/Cancelled, **backup** **`version`** **`3`** (**`paymentPlans`**), **renewal** and **prescription** reminder banners on main screens; **Profile** / **Sign out** / **Upcoming renewals** in the **avatar menu**; **amber count badge** beside the avatar toggles renewal tables when reminders qualify). [Renewals](./docs/RENEWALS.md) (category **`renewal`**, **`renewal_kind`**, import staging, **`/renewals`**, vs. **Expenses** table; combined **Projection** uses **Active** rows only). [Payment plans](./docs/PAYMENT_PLANS.md) (**`/payment-plans`**, **`payment_plans`**, **`payment_plan`** expense sync, **Add payment plan** Show/Hide and auto-collapse). [Prescriptions](./docs/PRESCRIPTIONS.md) (**`/prescriptions`**, **`prescriptions`** table, **`renewal_period`** monthly **1–11** or **1–5 years**, **`next_renewal_date`**, ~**30-day** in-app reminders). [Architecture](./docs/ARCHITECTURE.md) and [Architecture diagrams](./docs/ARCHITECTURE_DIAGRAM.md) (OAuth, topology, data model including **`expenses.website`** / **`renewal_kind`**, **`prescriptions`**, **`POST /auth/refresh`**, backup routes and **409** cross-account restore, **`recovery_code_ciphertext`**, **recurring metadata derived from transaction date**, **renewal reminder routes**). [PM2 and controlling applications](./docs/HOWTO_CONTROLLING_APPLICATIONS.md). **[Deployment](./deployment/README.md)** (Docker Compose **`npm run compose:prod`**, Kubernetes).
...
- **Profile and recovery (authenticated):** `PATCH /api/auth/profile`, `POST`/`DELETE /api/auth/avatar`, `POST`/`DELETE /api/auth/recovery-code` (stores encrypted ciphertext for backup export). **`POST /api/auth/recover-password`** (no auth) resets password from a saved recovery code. **`GET /api/backup/export`** and **`POST /api/backup/restore`** export or import JSON backups (`version` **`1`**–**`3`**; **`2`** adds **`prescriptions`**; **`3`** adds **`paymentPlans`**; renewals remain **`expenses`** with **`category`** **`renewal`**). `append` or `replace`: scope depends on **`version`** (**`3`** **replace** also clears **`payment_plans`**). **`account`** object with **`userId`**, **`email`**, **`label`**, **`hasRecoveryCode`**, optional **`recoveryCode`**; download filename includes the account email; cross-account restore returns **409** `BACKUP_ACCOUNT_MISMATCH` unless the client sends **`confirmCrossAccountRestore`**: **`true`**). Restore body limit 15 MB, up to 25,000 rows per array; expense objects include **`state`** (`active`, `paused`, or `cancelled`, matching the database), plus `payment_day` / `payment_month` from `spent_at`. See [docs/USER_GUIDE.md](./docs/USER_GUIDE.md) and [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md).
