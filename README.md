...
**Documentation index:** [User guide](./docs/USER_GUIDE.md) (accounts, import, **Renewals** screen, **Expenses** list excluding renewal-category rows, reports, **recovery codes**, **backup/restore** with **`account`** labeling and optional **`recoveryCode`** in JSON, **expired-session refresh**, **production Compose** / **`ensure-env` JWT bootstrap**, expense **State** Active/Cancel, **renewal reminders** on all main screens—**grouped by institution**, **sortable** expense and renewal tables; **Profile** / **Sign out** / **Upcoming renewals** in the **avatar menu**; **amber count badge** beside the avatar toggles renewal tables when reminders qualify). [Renewals](./docs/RENEWALS.md) (category **`renewal`**, **`renewal_kind`**, import staging, **`/renewals`**, vs. **Expenses** table). [Architecture](./docs/ARCHITECTURE.md) and [Architecture diagrams](./docs/ARCHITECTURE_DIAGRAM.md) (OAuth, topology, data model including **`expenses.website`** / **`renewal_kind`**, **`POST /auth/refresh`**, backup routes and **409** cross-account restore, **`recovery_code_ciphertext`**, **recurring metadata derived from transaction date**, **renewal reminder routes**). [PM2 and controlling applications](./docs/HOWTO_CONTROLLING_APPLICATIONS.md). **[Deployment](./deployment/README.md)** (Docker Compose **`npm run compose:prod`**, Kubernetes).
