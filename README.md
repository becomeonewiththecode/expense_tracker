...
- **Profile and recovery (authenticated):** `PATCH /api/auth/profile`, `POST`/`DELETE /api/auth/avatar`, `POST`/`DELETE /api/auth/recovery-code` (stores encrypted ciphertext for backup export). **`POST /api/auth/recover-password`** (no auth) resets password from a saved recovery code. **`GET /api/backup/export`** and **`POST /api/backup/restore`** export or import JSON backups (`version` **`1`** or **`2`**; **`2`** adds **`prescriptions`** and **`renewalCount`**; renewals are **`expenses`** with **`category`** **`renewal`**). `append` or `replace`: **`replace`** with **`version`** **`1`** only replaces **expenses**; **`replace`** with **`version`** **`2`** replaces **expenses** and **prescriptions**. **`account`** object with **`userId`**, **`email`**, **`label`**, **`hasRecoveryCode`**, optional **`recoveryCode`**; download filename includes the account email; cross-account restore returns **409** `BACKUP_ACCOUNT_MISMATCH` unless the client sends **`confirmCrossAccountRestore`**: **`true`**). Restore body limit 15 MB, up to 25,000 rows per array; expense objects include `state` (`active` / `cancel`), `payment_day` and `payment_month` derived from `spent_at`—older files without `state` restore as `active`. See [docs/USER_GUIDE.md](./docs/USER_GUIDE.md) and [docs/ARCHITECTURE.md](./docs/ARCHITECTURE.md).
